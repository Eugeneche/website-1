...
>>>>> ВАЖНО! >>>>> !!! ПРОВЕРИТЬ НАЛИЧИЕ КАТЕГОРИИ В ФАЙЛЕ order.mdx (/categories/order.mdx), иначе категория не отобразится на главной странице!
...
>>>>> ВАЖНО! >>>>> !!! В сайте - примере 3 колонки для проектов. Если в "show_in_main_page" отметить "yes" больше 3-х проектов, то они будут располагаться вторым (третьим и т.д.) рядом.
...
Чтобы отредактировать страницу "Obchodní podmínky": 
---

Заходим на страницу /pages/terms-and-conditions.js - там находится весь текст с условиями, можно править прямо на Github. Если в какой-либо раздел нужно добавть новый параграф, оборачиваем его в теги `<p></p>` для визуального структурирования. Если какой-либо параграф нужно удалить, удаляем его вместе в тегами `<p></p>`, которые его оборачивают.

Если нужно добавить новый раздел со своими параграфами, заголовок нового раздела оборачиваем в теги `<h4></h4>`, а его параграфы - в теги `<p></p>`:

`<h4>Заголовок вновь созданного раздела</h4>`<br>

`<p>Параграф первый</p>`<br>
`<p>Параграф второй</p>`<br>
`<p>Параграф третий</p>`<br>

Внизу подтверждаем кликом кнопки Commit changes.

---

Чтобы отредактировать страницу "Zásady ochrany osobních údajů": 
---

Заходим в папку /src/terms_and_conditions и там находятся языковые версии текста. Аналогично странице "Obchodní podmínky", заголовки оборачиваются в теги `<h4></h4>` (при необходимости - подзаголовки в `<h5></h5>`), параграфы - в `<p></p>`.

Внизу подтверждаем кликом кнопки Commit changes.

---

Чтобы отредактировать страницу "Fotoateliér": 
---

Тексты находятся в файлах i18/l10n/cs.json или en.json или ru.json соответственно.

Фото находятся в папке /src/images/studio. Их можно "перетягивать" прямо в папку на Github, либо удалять прямо из Github.
